...
class DigraphTest : public Test {
    ...
    public:
        DigraphTest(std::string path = "test.log", std::string name = "", std::string description = "") : Test(path, name, description) {
        ...
        bool edgesTest() {
            return graph.getEdges().size() == 20;
        }

        bool degreeTest() {
            return graph.degree("A").first() == 1 && graph.degree("A").second() == 2;
        }

        bool neighborsTest() {
            return graph.getNeighbors("A").size() == 2;
        }
        ...
        bool run() {
            ...
            file << (this->name.length() > 0 ? this->name : "Digraph Test:") << std::endl;
            file << (this->description.length() > 0 ? this->description : "\tTest for the digraph datastruct.") << std::endl;
